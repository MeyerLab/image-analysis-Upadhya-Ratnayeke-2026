...
settings.conditions = {
    %row, column, site, frame drug added
       ...
       'siCtrl DMSO',2:4,2,1,[0 0 0],[0 10]; %1  
       % 'siCtrl DOX',5:8,2,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'siCCNA2 DMSO',2:4,3,1,[0 0 0],[0 10]; %1  
       % 'siCCNA2 DOX',5:8,3,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'siCCNE1+2 DMSO',2:4,4,1,[0 0 0],[0 10]; %1  
       % 'siCCNE1+2 DOX',5:8,4,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'siCCNB1+2 DMSO',2:4,5,1,[0 0 0],[0 10]; %1  
       % 'siCCNB1+2 DOX',5:8,5,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'siSkp2 DMSO',2:4,6,1,[0 0 0],[0 10]; %1  
       % 'siSkp2 DOX',5:8,6,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'siCDC25a DMSO',2:4,7,1,[0 0 0],[0 10]; %1  
       % 'siCDC25a DOX',5:8,7,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'sip21 DMSO',2:4,8,1,[0 0 0],[0 10]; %1  
       % 'sip21DOX',5:8,8,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'sip27 DMSO',2:4,9,1,[0 0 0],[0 10]; %1  
       % 'sip27 DOX',5:8,9,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'siEmi1 DMSO',2:4,10,1,[0 0 0],[0 10]; %1  
       % 'siEmi1 DOX',5:8,10,1,[0 0 0],[0 10]; %1  
       % 
       % 
       %  'siRif1 DMSO',2:4,11,1,[0 0 0],[0 10]; %1  
       % 'siRif1 DOX',5:8,11,1,[0 0 0],[0 10]; %1  
       % 
...
%%% Paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
settings.dataDir = 'C:\Users\swu4001\Desktop\Data\A041\A041-live\Data';
...
settings.saveName = 'A041_data_apcTrace_example.mat';
settings.saveDir = 'C:\Users\swu4001\OneDrive - med.cornell.edu\APC_Paper\Figures_Revised\mat files\Fig1_S1';
%%% Analysis Options %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(S)
    ...
    if settings.IFoption
        ...
        S(i).iRFP2cyt = S(i).IFdata(:, ismember(IFnames,'2_iRFP_cyto ring'));
         ...
         S(i).RFP202 = S(i).IFdata(:, ismember(IFnames,'2_20xRFP_mean'));
        S(i).RFP202cyt = S(i).IFdata(:, ismember(IFnames,'2_20xRFP_cyto ring'));
...
%         S(i).apcNuc = fillTraceVals(S(i).apcNuc, S(i).traceStats, 5);
        ...
        if strcmp(settings.type, 'cycling') | strcmp(settings.type, 'hybrid')
            % Divided cells
            minMaxThresh = [200 1000]; %[number that the minimum of each trace has to be below          number max of each trace has to be above]
            ...
            if strcmp(settings.type,'cycling') | strcmp(settings.type,'hybrid')
             ...
             [S(i).POI(subset,2), badTracesAPC(subset)] = findAPCInact(S(i).apcAreaNormM(subset,:), S(i).traceStats(subset,:), ...
                struct('postBuffer',10,'smooth',5,'cycling',1,...
                        'buff',1,'thresh',.002,'preBuffer',5,'lowThresh',.2, 'increase',.004 *10 ,...
...
%         S(i).crlNuc = fillTraceVals(S(i).crlNuc, S(i).traceStats, 5);
        ...
        maskTrace = [];
        %meanWindow = 20:30;
        lowThresh = 250; %max has to be above
        noiseMask = ones(1,numFrames-1);
        noiseMask(maskTrace)=0;
...
%             inds = find(highNoise);
        ...
        if settings.poiCrl          
            ...
            S(i).POI(subset,3:4) = findCRL4Act_v2_reverse(struct('CRLtrace',S(i).crlArea(subset,:),...
                ...
                'firstD',0,'secD',.000,'decrease', .05, 'minSlope', -.03, 'early', 5,'falseCall', .8, 'cytoCheck', 0,'minExp',50,...
                'trunk',.4,'debug',0));

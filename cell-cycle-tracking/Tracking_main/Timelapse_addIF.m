...
%     try
        ...
        switch settings.microscope
            ...
            case 'nikon'
                ...
                live.rawprev( live.rawprev==0) = prctile( live.rawprev(:),10); 
        end
    ...
    for j = 1:length(names{i})
         switch settings.microscope
            ...
            case 'nikon'
               ...
               raw{i}{j}(raw{i}{j}==0) = prctile(raw{i}{j}(:),10);
         end
...
for i=1:length(names)
    for j = 1:length(names{i})
        ...
        switch settings.bgsubmethod{i}{j}
            case 'global nuclear'
                real{i}{j} = bgsubmasked_global_2(blur,nanmask_aligned{i}{j},1,compression,settings.bgperctile{i}(j));
                if settings.punctacalc{i}(j)
                    unblurred{i}{j} = bgsubmasked_global_2(raw{i}{j},nanmask_aligned{i}{j},1,compression,settings.bgperctile{i}(j));
                end
            case 'global cyto'
...
livetrack(:,1:2) = livetrack(:,1:2);
% livetrack(:,3:4) = livetrack(:,3:4)*settings.scaleLive^2;
livetrack(:,3:4) = livetrack(:,3:4);

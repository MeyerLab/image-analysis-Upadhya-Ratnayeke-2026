...
%updated 2025 for matlab compatibility 
...
% centers_idx=sub2ind([height,width],centers(:,2),centers(:,1)); %old command line

   centers_idx=NaN(size(centers,1),1); %initialize centers_idx with nan values
   valid_idx=~isnan(centers(:,1)); % find non-nan indices
   valid_centers=centers(valid_idx,:); % find non-nan centers
   centers_idx(valid_idx)=sub2ind([height,width],valid_centers(:,2),valid_centers(:,1)); 
